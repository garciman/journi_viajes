...
void main() {
  test('list devuelve una lista inmodificable', () async {
    ...
    expect(() => list.add(_mk(id: 'x', trip: 't1', ts: DateTime.utc(2025))),
        throwsUnsupportedError);
  });

  test('deleteById es idempotente y findById devuelve null si no existe',
      () async {
    final repo = InMemoryEntryRepository();
  ...
  test('orden por createdAt DESC y filtros por tripId/type', () async {
    ...
    final e1 = _mk(
        id: 'e1',
        trip: 't1',
        ts: t0.subtract(const Duration(seconds: 5)),
        type: EntryType.note);
    ...
    final onlyPhotos =
        (await repo.list(tripId: 't1', type: EntryType.photo)).asOk().value;
  ...
  test('watchAll emite snapshot inicial y cambios', () async {
    ...
    final expectation = expectLater(
      ...
      emitsInOrder([
        ...
        isA<List<Entry>>()
            .having((l) => l.map((e) => e.id).toList(), 'after add', ['e1']),
