...
void main() {
  ...
  test('authenticate ok y wrong password falla', () async {
    ...
    final ok = await service.authenticate(
      const AuthenticateUserCommand(email: 'ADA@EXAMPLE.COM', password: 'pass123'),
    );
    expect(ok.isOk, isTrue);

    // login incorrecto
    final bad = await service.authenticate(
      const AuthenticateUserCommand(email: 'ada@example.com', password: 'wrong'),
    );
  ...
  test('duplicate email en register -> Err(RepoError)', () async {
    final c1 = RegisterUserCommand(
      id: 'u1', name: 'A', lastName: 'B', email: 'dup@example.com', password: 'x');
    final c2 = RegisterUserCommand(
      id: 'u2', name: 'C', lastName: 'D', email: 'dup@example.com', password: 'y');
